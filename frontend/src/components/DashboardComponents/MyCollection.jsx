    ...
    <div className="bg-white border border-slate-200/60 p-6 sm:p-8 rounded-2xl shadow-sm">
            ...
            <div key={book._id} className="group border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 bg-slate-50/20 flex flex-col h-full">
              ...
              <div className="p-4 flex flex-col flex-grow justify-between">
                ...
                <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-100 text-xs font-semibold">
                  <Link 
                    to={`/books/${book._id}`} 
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 border border-slate-200 hover:bg-slate-50 text-slate-650 rounded-lg transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 border border-slate-200 hover:bg-amber-50 hover:text-amber-800 text-slate-650 rounded-lg transition-colors cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="p-2 border border-slate-200 hover:bg-red-50 hover:text-red-600 text-slate-650 rounded-lg transition-colors cursor-pointer"
                  >
      ...
      {editingBook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden text-left flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <button 
                ...
                className="text-slate-450 hover:text-slate-650 transition-colors p-1.5 rounded-full hover:bg-slate-100 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="flex-grow overflow-y-auto p-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-750 mb-1.5">Book Title</label>
                <input
                  type="text"
                  required
                  value={editForm.title}
                  onChange={(e) => setEditForm({ ...editForm, title: e.target.value })}
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
                />
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-750 mb-1.5">Price (₹)</label>
                <input
                  type="number"
                  required
                  min="0"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-755 mb-1.5">Genres</label>
                <select
                  multiple
                  required
                  ...
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100 min-h-[120px]"
                >
                ...
                <p className="text-[10px] text-slate-450 mt-1">Hold Ctrl (or Cmd) to select multiple genres.</p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-750 mb-1.5">Description</label>
                <textarea
                  required
                  rows="4"
                  value={editForm.description}
                  onChange={(e) => setEditForm({ ...editForm, description: e.target.value })}
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100 resize-none"
                />
              </div>

              {modalError && (
                <div className="flex items-center gap-2 text-rose-650 text-xs font-semibold">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button
                  ...
                  onClick={() => setEditingBook(null)}
                  disabled={submitting}
                  className="px-5 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-650 rounded-full text-xs font-bold transition-colors cursor-pointer disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2.5 bg-amber-700 hover:bg-amber-800 text-white rounded-full text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                >
                  {submitting ? (
                    <>
      ...
      {deletingBook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden text-left p-6 space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
              <button
                ...
                disabled={submitting}
                className="px-5 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-655 rounded-full text-xs font-bold transition-colors cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={submitting}
                className="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-full text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
              >
                {submitting ? (
                  <>
                    <Loader className="w-3.5 h-3.5 animate-spin" />
